...
for j = 1:size(Theta1,1)
  for k = 2:size(Theta1,2)
    Reg_cost = Reg_cost + Theta1(j,k)^2;
  endfor
endfor

for j = 1:size(Theta2,1)
  for k = 2:size(Theta2,2)
    Reg_cost = Reg_cost + Theta2(j,k)^2;
  endfor
endfor
Reg_cost = lambda * Reg_cost / (2 * m);
J = J + Reg_cost;
...
for t = 1:m
  ...
  z3_t = a2_t * Theta2';
  H_theta_t = sigmoid(z3_t);
  
  delta3 = zeros(num_labels,1);
  for k = 1:num_labels
    delta3(k) = H_theta_t(k) - Y(t, k);
  endfor
  
  delta2 = Theta2' * delta3;
  delta2 = delta2(2:end);
  delta2 = [delta2 .* sigmoidGradient(z2_t')];
